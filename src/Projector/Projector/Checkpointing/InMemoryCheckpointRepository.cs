...
public class InMemoryCheckpointRepository : ICheckpointRepository
{
    ...
    public Task CreateCheckpoint(Checkpoint checkpoint, CancellationToken cancellationToken = default)
    {
        _checkpoints.AddOrUpdate(checkpoint.ProjectionName, _ =>  checkpoint, (_, _) => checkpoint);
        return Task.CompletedTask;
    }

    public Task UpdateCheckpoint(Checkpoint checkpoint, CancellationToken cancellationToken = default)
    {
        _checkpoints.AddOrUpdate(checkpoint.ProjectionName, _ =>  checkpoint, (_, _) => checkpoint);
        return Task.CompletedTask;
    }
